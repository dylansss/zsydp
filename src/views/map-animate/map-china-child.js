...
export class ChildMap {
  ...
  addPointEvent() {
    ...
    this.pointEventElement.map((mesh) => {
      ...
      mesh.addEventListener("mouseover", (event) => {
        // console.log(event, 'addPointEvent-mouseover-event');
        let userData = event.target.userData
        this.allInfoLabel.map((label, index) => {
          label.hide()
          if (userData.index === index) {
      ...
      mesh.addEventListener("mouseout", (event) => {
        // console.log(event, 'addPointEvent-mouseout-event');
        let userData = event.target.userData
        this.allInfoLabel.map((label, index) => {
          label.hide()
        })
  ...
  labelNameStyle(data, index, position) {
    ...
    label.init(`<div class="area-name-label-wrap">${data.name}<br/>${data.value}</div>`, position)
